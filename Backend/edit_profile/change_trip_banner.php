...
$upload_dir = "../images/trip_banner/";

if (isset($_FILES['file']) && isset($_POST['trip_id'])) {
    $trip_id = $_POST['trip_id'];
    $file_name = time() . "_" . basename($_FILES['file']['name']); // Unique filename
    $target_file = $upload_dir . $file_name;

    // ✅ 1. Fetch the existing profile photo filename
    $query = "SELECT trip_banner FROM trip_details WHERE trip_id = '$trip_id'";
    $result = mysqli_query($conn, $query);
    if ($result) {
        $row = mysqli_fetch_assoc($result);
        $old_file = $row['trip_banner'];
    ...
    if (move_uploaded_file($_FILES['file']['tmp_name'], $target_file)) {
        $sql_update = "UPDATE trip_details SET trip_banner = '$file_name' WHERE trip_id = '$trip_id'";
